...
See [docs/product-roadmap.md](docs/product-roadmap.md) for current phase status, dependencies, exit criteria, and provisional direction tracked against the phases below.
...
2. **A schema synchronization can be added to.** Update time, a deletion tombstone, a revision, and an originating device on every table a person owns, plus a record of local changes not yet sent anywhere. No synchronization is built and nothing reaches a network. This is second on purpose: the migration is small, but making deletion a tombstone changes every read path that currently assumes a deleted row is gone, and that cost is paid once against today's tables or repeatedly against every table added before it. It is also the design work the offline-first philosophy above requires before synchronization may be implemented. Specified in [Specification 0042](specs/0042-schema-synchronization-readiness.md); see [ADR 0032](docs/decisions/0032-schema-synchronization-readiness.md) for why each shape was chosen over its alternative.
3. **A usable exercise library.** The library ships empty and offers twenty-six starter definitions, plus a further hundred eighty-nine curated from an openly licensed dataset that the person can add in a second, equally deliberate action — two hundred fifteen in total, with the attribution that dataset requires recorded in [docs/third-party-material.md](docs/third-party-material.md). A definition deleted from either pack can be explicitly restored in place, without discarding an edit made to it before the deletion. Specified in [Specification 0027](specs/0027-starter-exercise-library.md), [Specification 0043](specs/0043-expanded-exercise-library.md), and [Specification 0044](specs/0044-deliberate-exercise-pack-restoration.md).
4. **Training depth.** Rest timing within a session, effort recorded as reserve or exertion, estimated maxima, grouped sets, and progression schemes were candidate capabilities that would separate a log of what happened from a tool that informs what to do next. Rest timing and optional reps-in-reserve recording are now shipped, satisfying this phase's exit criterion; estimated maxima, grouped sets, and progression schemes remain unapproved candidates for a later phase. See [docs/product-roadmap.md](docs/product-roadmap.md) for current status and provisional direction.
5. **Nutrition depth.** A real food database behind the existing catalog, barcode entry, and macronutrient targets derived from the goal and energy calculations already shipped. Goal-derived macro targets are now shipped ([Specification 0047](specs/0047-goal-derived-macro-targets.md)); the food database and barcode entry remain unapproved. The initial nutrition launch market (United States) and independent sampling frames were defined and approved in Sprint 51 ([ADR 0037](docs/decisions/0037-initial-nutrition-market-and-independent-sampling-frame.md)), and Sprint 52 executed the resulting coverage evaluation: FoodData Central's branded-name and exact-barcode discovery both failed their required thresholds by a wide margin, so it is **not approved** ([ADR 0038](docs/decisions/0038-fooddata-central-coverage-evaluation-fails.md)). Sprint 53 authored a counsel-ready Open Food Facts licensing review packet ([docs/open-food-facts-licensing-review-packet.md](docs/open-food-facts-licensing-review-packet.md)) and recorded Outcome C in [ADR 0039](docs/decisions/0039-open-food-facts-licensing-boundary-review-packet.md): Open Food Facts remains unapproved pending external legal review, which remains the sole named path to unblock this track.
...
One question that blocked phase 3 was resolved in practice: how much exercise data ships inside the application. The complete multilingual dataset was far too large to bundle, and Specification 0043 answered it by curating a single-language, image-free 189-entry pack from an openly licensed dataset rather than trading a smaller catalog against images fetched on first use — no image is bundled or fetched at all. Phase 5 splits into two independently sequenced tracks. Macronutrient targets needed no food-product data at all and shipped in Sprint 49 as [Specification 0047](specs/0047-goal-derived-macro-targets.md): a person with a saved goal sees a protein, carbohydrate, and fat target beside their calculated daily calorie target, derived entirely on-device from values already computed there. A real food database behind the existing catalog, with barcode entry, remains open. [ADR 0035](docs/decisions/0035-nutrition-provenance-and-unapproved-food-data-sourcing.md) records that Open Food Facts's ODbL 1.0 license carries a share-alike obligation whose reach over a subset bundled inside a distributed application, rather than published as its own derived database, is not settled by the license's own documentation and needs qualified legal review. Sprint 50 ran FoodData Central's alternative coverage path: its April 2026 release had strong internal nutrition completeness, but no independent representative food/barcode sample or named launch market existed, so real-world discovery stayed unproven and [ADR 0036](docs/decisions/0036-fooddata-central-coverage-remains-unproven.md) kept it unapproved. Sprint 51 defined the United States as the initial launch market and approved independent sampling frames ([ADR 0037](docs/decisions/0037-initial-nutrition-market-and-independent-sampling-frame.md)), establishing the prerequisites to execute the external discovery measurement under Sprint 50's fixed thresholds. Sprint 52 executed that measurement against 385 independently sampled items per stratum: ordinary-food discovery passed, but branded-name discovery (31.43%) and exact-barcode discovery (55.58%) both failed their required 80% thresholds by a wide margin, so FoodData Central is **not approved** ([ADR 0038](docs/decisions/0038-fooddata-central-coverage-evaluation-fails.md)). Sprint 53 authored the counsel-ready licensing review packet ([docs/open-food-facts-licensing-review-packet.md](docs/open-food-facts-licensing-review-packet.md)) and recorded Outcome C in [ADR 0039](docs/decisions/0039-open-food-facts-licensing-boundary-review-packet.md). Neither provider is approved; qualified Open Food Facts legal review is now the sole remaining named path, so a future sprint proceeds from measured evidence rather than assumptions.
