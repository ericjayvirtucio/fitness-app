...
4. **Training depth (current phase).** Rest timing within a session, effort recorded as reserve or exertion, estimated maxima, grouped sets, and progression schemes are candidate capabilities that would separate a log of what happened from a tool that informs what to do next. None is yet approved as scope. See [docs/product-roadmap.md](docs/product-roadmap.md) for the current discovery and provisional direction.
