...
- **5. Nutrition depth** — logging a food item finds it in a real database
  ...
  [Sampling frame evaluation](nutrition-sampling-frame-evaluation.md)). The food-database
  half remains unmet until the external evaluation is executed and passes all thresholds.
  This phase does not exit until both halves are independently met.)_
