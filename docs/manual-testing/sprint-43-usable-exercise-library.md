...
| Delete then import    | Delete one expanded-pack definition, then import the expanded pack again.                                                                                                     | **Not** silently restored. The import is refused ("could not be added, nothing was changed"), and the deleted definition stays deleted. See the note below — this is a change from Sprint 27's documented behavior for the starter set, caused by Sprint 42's tombstone-based deletion, and applies identically to the starter set today. |
...
> imported starter exercise and importing again adds it back. That was true
> before Sprint 42. Sprint 42 changed deletion for this table from a hard
> delete to a tombstone (`deleted_at_epoch_ms`), and the import's
> duplicate-identifier check only reads non-deleted rows — so a re-import now
> tries to `INSERT` a row whose identifier still physically exists (deleted or
> not) and collides with the primary key, refusing the whole import instead of
> reviving the one row. The net effect a person sees is that the deletion
> holds (nothing is silently undone), but the message is a generic refusal
> rather than a specific "already deleted" statement, and it applies to the
> starter set too, not only the expanded pack. This is a pre-existing
> regression from Sprint 42, not something Sprint 43 introduced or has fixed —
> flag it if it surprises you, but do not fail this sprint's QA on it.
...
| Sync metadata      | After importing, inspect (or have engineering inspect) `updated_at_epoch_ms`, `revision`, and `originating_device_id` on a few expanded-pack rows. | Each is populated exactly as for a hand-authored row — no special case for imported content.                         |
