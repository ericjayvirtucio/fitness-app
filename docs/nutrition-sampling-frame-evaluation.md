...
| Market                              | Primary Language                     | Barcode Standard                 | Mandatory Nutrition Label Units                                                                 |                 Domain Model Fit                  | FoodData Central Branded Records | Independent Consumption / Assortment Data    |
| :---------------------------------- | :----------------------------------- | :------------------------------- | :---------------------------------------------------------------------------------------------- | :-----------------------------------------------: | :------------------------------: | :------------------------------------------- |
| **United States (US)**              | English (`en-US`)                    | UPC-A (GTIN-12), GTIN-8, GTIN-14 | Calories (kcal), Protein (g), Total Carb (g), Fat (g), Fiber (g), Total Sugars (g), Sodium (mg) |                **Exact 1:1 match**                |         429,150 (99.75%)         | CDC/USDA NHANES WWEIA (Public domain)        |
| **United Kingdom (UK)**             | English (`en-GB`)                    | EAN-13                           | Energy in kJ + kcal, declared strictly **per 100g/ml**, **Salt (g)** instead of Sodium (mg)     | Mismatch (Requires Salt in grams & per-100g base) |            0 (0.00%)             | UK NDNS, McCance & Widdowson's CoFID         |
| **Canada (CA)**                     | English (`en-CA`) & French (`fr-CA`) | UPC-A, EAN-13                    | Calories (kcal), Protein (g), Carb (g), Fat (g), Fiber (g), Sugars (g), Sodium (mg) — bilingual |    Partial (Requires bilingual packaging & UI)    |            0 (0.00%)             | Health Canada CCHS Nutrition, CNF            |
| **Australia / New Zealand (AU/NZ)** | English (`en-AU`/`en-NZ`)            | EAN-13                           | Energy in **kJ** (mandatory), declared **per 100g/ml** and per serving, Sodium (mg)             |         Mismatch (Requires mandatory kJ)          |      1,090 NZ (0.25%), 0 AU      | AUSNUT 2011–13, AFCD                         |
| **European Union (EU)**             | Multilingual (DE, FR, ES, IT, etc.)  | EAN-13                           | Energy in kJ + kcal, declared **per 100g/ml**, **Salt (g)** instead of Sodium (mg)              | Mismatch (Requires Salt in grams & per-100g base) |            0 (0.00%)             | EFSA Comprehensive Food Consumption Database |
| **Philippines (PH)**                | Filipino / English (`en-PH`)         | EAN-13 (prefix 480), UPC-A       | Calories (kcal), Protein (g), Carb (g), Fat (g), Sodium (mg)                                    |                     Moderate                      |            0 (0.00%)             | FNRI Food Composition Tables                 |
| **Global / Multi-Market**           | All                                  | All (UPC/EAN)                    | Incompatible conflicting statutory formats                                                      |                       Poor                        |              Mixed               | No single representative global frame exists |
...
- **Domain model alignment:** The pure domain model in `packages/domain/src/nutrition/nutrition-facts.ts` defines nutrients as `energy` (kcal), `protein` (g), `carbohydrate` (g), `fat` (g), `fiber` (g), `sugars` (g), and `sodium` (mg). This corresponds exactly to the mandatory elements of the United States FDA Nutrition Facts label (21 CFR 101.9).
- **Measurement units:** US packaging displays sodium directly in milligrams (`mg`), which matches the application's canonical unit without mathematical conversion.
- **Language:** The application's interface, error text, and design-system typography are built in American English (`en-US`), matching US grocery terminology (e.g., "ground beef", "bell pepper", "zucchini").
- **Barcode infrastructure:** The US retail supply chain is dominated by 12-digit Universal Product Codes (UPC-A / GTIN-12), which the application's GTIN parser and check-digit validator already normalize and validate.
- **Independent data availability:** The US federal government provides the world's most rigorous, unencumbered, public-domain dietary recall survey: the CDC/USDA NHANES What We Eat in America (WWEIA) dataset.

#### 2. United Kingdom (UK) and European Union (EU) — Excluded from Initial Launch

- **Statutory unit mismatch:** Under UK and EU FIC Regulation No 1169/2011, nutrition panels are legally required to declare energy in kilojoules (`kJ`) alongside kilocalories (`kcal`), declare values strictly **per 100g or per 100ml** (with serving sizes optional), and declare **salt in grams** (`salt = sodium × 2.5`) rather than elemental sodium in milligrams.
- **Domain impact:** Serving these markets truthfully would require expanding `NutritionFacts` to support salt vs. sodium distinctions and kilojoules, or risk misleading users with inaccurate label representations.
- **Localization:** EU markets require immediate multi-language translation (French, German, Spanish, etc.), violating Phase 3's single-language scope.

#### 3. Canada (CA) — Excluded from Initial Launch

- **Bilingual labeling mandate:** Health Canada regulations require bilingual English and French food packaging and labeling. The application is currently single-language English.
- **Provider coverage:** Zero Canadian records exist in FoodData Central Branded bulk releases.

#### 4. Australia / New Zealand (AU/NZ) — Excluded from Initial Launch

- **Energy unit mandate:** Food Standards Australia New Zealand (FSANZ) mandates kilojoules (`kJ`) as the primary energy metric.
- **Provider coverage:** Australia has 0 records in FoodData Central; New Zealand has 1,090 records (0.25%), insufficient for standalone launch.
